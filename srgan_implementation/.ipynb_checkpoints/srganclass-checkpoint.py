...
class Generator(nn.Module): 
    def __init__(self, in_channels = 3, num_channels = 64, num_blocks =16 ): 
        super().__init__()
        self.initial = ConvLayer(in_channels, num_channels, kernel_size = 9, stride = 1, padding = 4, use_bn = False)
        self.residuals = nn.Sequential(*[ResidualBlock(num_channels) for _ in range (num_blocks)])
        self.convblock = ConvLayer(num_channels, num_channels, kernel_size = 3 ,  padding = 1, stride =1, use_act = False)
...
class ConvLayer(nn.Module):
    def __init__(
        ...
        **kwargs 
        ...
        #nn.identity returns unchanged input 
...
class UpsampleBlock(nn.Module): 
    def __init__(self, in_channels, scale_factor): 
        ...
        #manually setting the kernel size, stride and padding 
...
class ResidualBlock(nn.Module): 
    ...
    def forward(self, x): 
        ...
        #transformed data + skip connection 
        return out + x 
